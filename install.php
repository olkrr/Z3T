...
<?php
 ...
 $query = 'DROP TABLE IF EXISTS `'.DB::$tableStatus.'`';
	...
	$query = 'CREATE TABLE IF NOT EXISTS `'.DB::$tableTypMyjki.'` (
		...
		`'.DB\TypMyjki::$TypMyjkiNazwa.'` VARCHAR(20) NOT NULL,
...
$query = 'CREATE TABLE IF NOT EXISTS `'.DB::$tableProducent.'` (
	...
	$query = 'CREATE TABLE IF NOT EXISTS `'.DB::$tableNaprawa.'` (
        ...
        `'.DB\Naprawa::$Wycena.'` BIT NOT NULL,
	...
	$query = 'CREATE TABLE IF NOT EXISTS `'.DB::$tableUser.'` (
		`'.DB\User::$Login.'` VARCHAR(20) UNIQUE NOT NULL,
		`'.DB\User::$Pass.'` VARCHAR(50) NOT NULL,
		PRIMARY KEY (Login)) ENGINE=InnoDB;';
    //`'.DB\User::$Uid.'` INT AUTO_INCREMENT, 
	try
	{
		$pdo->exec($query);
	}
...
//////////////////////////////////////////////////
	...
	try
	{
		$stmt = $pdo -> prepare('INSERT INTO `'.DB::$tableUser.'` (`'.DB\User::$Login.'`, `'.DB\User::$Pass.'`) VALUES(:Login, :Pass)');	
		foreach($Users as $User)
		{
			//strval($float), nie ma typu PDO::PARAM_FLOAT
			$stmt -> bindValue(':Login', $User['Login'], PDO::PARAM_STR);
			$stmt -> bindValue(':Pass', md5($User['Pass']), PDO::PARAM_STR);
			$stmt -> execute();
		}
