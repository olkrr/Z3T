...
<h1>Dodawanie naprawy </h1>
<form action="http://<?php echo $_SERVER['HTTP_HOST']?>/<?php echo \Config\Website\Config::$subdir?>index.php?controller=Naprawa&amp;action=add" method="post">
    <input type="hidden" readonly name="IDNaprawy"  value="<?php echo $this->get('IDNaprawy') ?>"> 
	
	ID Klienta: <input type="text" name="idKlienta" readonly value="<?php echo $this->get('IDKlient') ?>"/><br />
...
Model: <input type="text" name="NazMyjki" value="<?php echo $this->get('NazMyjki') ?>" /><br />
	...
	Wycena: <input type="text" name="Wycena" value="<?php echo $this->get('Wycena') ?>" /><br />
	  Przyjęto: <input type="date" name="DataDost" required="required"    placeholder="RRRR-MM-DD"  /><br />
	  Odebrano: <input type="date" name="DataOdbi"   placeholder="RRRR-MM-DD"    /><br />
	  Cena: <input type="text" name="KwotaNap" /><br />
 ...
 <label>Status:</label>
	...
	Opis: <input type="textarea" rows="4" cols="50" name="OpisNapr"  /><br/>
