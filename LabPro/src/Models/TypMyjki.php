<?php
    ...
    class TypMyjki extends Model
    {
        ...
        public function getAllForSelect()
        {
            ...
            $typymyjki[$przedmiot[\Config\Database\DBConfig\TypMyjki::$IDTypMyjki]] = $przedmiot[\Config\Database\DBConfig\TypMyjki::$TypMyjkiNazwa];
            return $typymyjki;
        ...
        public function add($TypMyjkiNazwa)
        {
            if($this->pdo === null)
            {
                $data['error'] = \Config\Database\DBErrorName::$connection;
            ...
            try
            {
				...
				.\Config\Database\DBConfig\TypMyjki::$TypMyjkiNazwa.'`) VALUES (:TypMyjkiNazwa)');                   

                $stmt->bindValue(':TypMyjkiNazwa', $TypMyjkiNazwa, PDO::PARAM_STR); 
                $result = $stmt->execute(); 
                if(!$result)
        ...
        public function update($IDTypMyjki, $TypMyjkiNazwa)
        {
            $data = array();
            if($this->pdo === null){
                $data['error'] = \Config\Database\DBErrorName::$connection;
            ...
            if($IDTypMyjki === null || $TypMyjkiNazwa === null){
                $data['error'] = \Config\Database\DBErrorName::$empty;
                return $data;
            }
            try	{
                $stmt = $this->pdo->prepare('UPDATE  `'.\Config\Database\DBConfig::$tableTypMyjki.'` SET
                `'.\Config\Database\DBConfig\TypMyjki::$TypMyjkiNazwa.'`=:TypMyjkiNazwa WHERE `'
